...
func proposalCmd(parseConfig *parsecmdtypes.Config) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			govModule := gov.NewModule(sources.GovSource, distrModule, mintModule, slashingModule, stakingModule, parseCtx.EncodingConfig.Marshaler, db)
			...
			err = govModule.UpdateProposalStatus(height, time.Now(), proposalID)
			if err != nil {
				return err
			}
...
func refreshProposalVotes(parseCtx *parser.Context, proposalID uint64, govModule *gov.Module) error {
	...
	for _, tx := range txs {
		...
		for index, msg := range junoTx.GetMsgs() {
			if msgVote, ok := msg.(*govtypes.MsgVote); !ok {
				continue
			} else {
